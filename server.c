...
int main(void)
{
	...
	fd_set sfd_set;
	FD_ZERO(&sfd_set);
	int max_sfd = MAX(srv_sfd_tcp, srv_sfd_udp) + 1;

	while(1) {
		FD_SET(srv_sfd_tcp, &sfd_set);
		FD_SET(srv_sfd_udp, &sfd_set);

		int sfd_ready = select(max_sfd, &sfd_set, NULL, NULL, NULL);
		if (sfd_ready  < 0) {
			if (errno == EINTR)
				continue;
			else
		...
		if (FD_ISSET(srv_sfd_tcp, &sfd_set))
			process_tcp_request(srv_sfd_tcp);

		if (FD_ISSET(srv_sfd_udp, &sfd_set))
